...
@api.route("/delete_post/<regex('[0-9a-f]{32}'):id>", methods=["GET"])
@ttl_redirect_user
@ttl_jwt_authentication
@ttlLimiter.limit_user(limit_value="10/day")
def api_delete_post(id):
    if ttl_check_user_agent() and len(id) == 32:
        return redirect(f"{CONSTANTS.ADMIN_URL}/admin/posts/delete/{id}")
        # {CONSTANTS.ADMIN_URL}
    else:

        return jsonify(message="This mode have not been unlocked."),200
        # decoded_dict = decoded_jwt()
        ...
        # storage.delete_blob(
        #     bucket_name = CONSTANTS.STORAGE_BUCKET_NAME,
        #     blob_name = decoded_dict["role"] + "/" + name + "/post/" + id + ".json",
        # )
...
def api_create_media():
    ...
    return redirect("authorised_user.media_upload", id=media_id)
